...
export const decrease = createAction(DECREASE) // export 함수를 통해 해당 타입 반환
...
const counterReducer = handleActions({ // 내부 리듀서는 아직 작동하지 않고 있음. 이벤트가 주어지기 전에는 선언만 되어 있음
  [INCREASE] : (state, action) => ({ number : state.number + 1 }), // Reducer 함수는 이전 상태와 액션 객체를 파라미터로 받음. state..number : 전역 state
