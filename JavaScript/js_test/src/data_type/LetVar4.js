function LetVar4() {
    ...
    // 호이스팅은 모든 변수가 프로그램 시작 시
    // 최선두에 선언하는 형식과 같이 동작하는 것을 의미한다.
    // 좀 더 쉽게 말하자면 var는 변수 선언과 값의 할당이 통합된다.
    // 반면 let은 변수 선언과 값의 할당이 분리되어 있다.

    // 그러다 보니 var는 변수 생성 시 이름이 같은 게 있으면
    // Mangling 이라는 방식을 통해서 변수의 이름을 바꿔버린다.
    // 반면 let은 선언과 할당이 분리되어 있다 보니
    // 왜 같은 것을 선언하냐면서 문법 오류가 나는 것이다.
    
    // var 대신 let을 도입하며 좋아진 점
    // 신입 개발자들이 var로 같은 변수를 도배하다 보니
    // 이전에 중요한 정보가 있었는데 그 정보가 덮어쓰기 되는 일이 많았다.
    // 반면 중요 정보를 let 으로 만들어 둠으로써
